...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        pos_dot = torch.bmm(output_vectors.unsqueeze(1), input_vectors.unsqueeze(2)).squeeze()
        out_loss = torch.nn.functional.logsigmoid(pos_dot).squeeze()


        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        neg_dot = torch.bmm(noise_vectors, input_vectors.unsqueeze(2)).squeeze()
        noise_loss = torch.nn.functional.logsigmoid(-neg_dot).sum(1)
